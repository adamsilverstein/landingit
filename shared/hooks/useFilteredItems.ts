...
export function useFilteredItems({ items, defaultFilter, defaultSort, isUnseen, staleDays, storage }: UseFilteredItemsOptions) {
  ...
  useEffect(() => {
    storage.getItem(STORAGE_KEYS.LABEL_FILTERS).then((stored) => {
      if (!stored) return;
      try {
    ...
    storage.getItem(STORAGE_KEYS.PR_STATE_FILTERS).then((stored) => {
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          if (valid.length > 0) {
            setPRStateFilters(new Set<PRStateFilterKey>(valid));
          }
        }
      } catch { /* ignore */ }
    });
  ...
  // Persist PR state filters to storage
  useEffect(() => {
    storage.setItem(STORAGE_KEYS.PR_STATE_FILTERS, JSON.stringify([...prStateFilters])).catch(() => {});
  }, [prStateFilters, storage]);

  // Persist label filters to storage
  useEffect(() => {
